...
class UploadController extends Controller
{
    ...
    public function actionFile()
    {
        ...
        $plan = (int)\Wskm::post('plan', 0);
        $isThumb = (bool)\Wskm::post('thumb', false);
        if ($plan === 1) {
            ...
            $ok = \wskm\Image::thumbnail($savePath, $savePath, $thumbWidth, $thumbHeight, \service\Setting::getConf('image', 'quality') ? (int)\service\Setting::getConf('image', 'quality') : 50);
            if ($ok === false) {
                Yii::error('Upload Avatar thumb error:'.$savePath);
            }
        } else if ($isThumb && $imgWidth && $imgHeight) {
            $thumbHeight = (int)\service\Setting::getConf('image', 'articleCoverHeight');
            $thumbWidth = (int)\service\Setting::getConf('image', 'articleCoverWidth');
            $thumbHeight = $thumbHeight ? $thumbHeight : 120;
            $thumbWidth = $thumbWidth ? $thumbWidth : 75;

            if ($imgWidth > $thumbWidth || $imgHeight > $thumbHeight) {
                $ok = \wskm\Image::thumbnail($savePath, $savePath, $thumbWidth, $thumbHeight, \service\Setting::getConf('image', 'quality') ? (int)\service\Setting::getConf('image', 'quality') : 50);
                if ($ok === false) {
                    Yii::error('Upload thumb error:'.$savePath);
                }
            ...
            $thumbHeight = (int)\service\Setting::getConf('image', 'imageMaxHeight');
            $thumbWidth = (int)\service\Setting::getConf('image', 'imageMaxWidth');
            $thumbHeight = $thumbHeight ? $thumbHeight : 1000;
            $thumbWidth = $thumbWidth ? $thumbWidth : 1000;

            if ($imgWidth > $thumbWidth || $imgHeight > $thumbHeight) {
                $ok = \wskm\Image::thumbnail($savePath, $savePath, $thumbWidth, $thumbHeight, \service\Setting::getConf('image', 'quality') ? (int)\service\Setting::getConf('image', 'quality') : 50);
                if ($ok === false) {
                    Yii::error('Reset image error:'.$savePath);
                }
        ...
        $modelFile->width = $imgWidth;
        $modelFile->height = $imgHeight;
