...
class ChineseAnalysis
{
    ...
    public static function filterInvalid(array $words)
    {
        return array_filter($words, function($v) {
            ...
            $v = strtolower($v);
            if (strpos('&nbsp', $v) !== false || strpos('&rdquo', $v) !== false
                || strpos('&ldquo', $v) !== false || strpos('&quot', $v) !== false 
                || strpos('&amp', $v) !== false || strpos('&rsquo', $v) !== false 
                || $v == 'lt' || $v == 'gt' || $v == 'the') {
                return false;
            }

            if (mb_strpos($v, '一') !== false || mb_strpos($v, '个') !== false || mb_strpos($v, '是') !== false 
                || mb_strpos($v, '些') !== false || mb_strpos($v, '的') !== false) {
                return false;
            }
